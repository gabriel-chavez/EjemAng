...
@Component({
  ...
  selector: '[app-row-providers-payments]',
  templateUrl: './row-providers-payments.component.html',
  styleUrls: ['./row-providers-payments.component.css'],
  providers: [ProvidersPaymentsService]
})
export class RowProvidersPaymentsComponent implements OnInit {
  ...
  constructor(private messageService: GlobalService, private providersPaymentsService: ProvidersPaymentsService) { }

  ngOnInit() {
    if (this.detail.documentType === undefined) {
      this.detail.documentType = this.constants.documentTypesProviders[0];
    } else {
      for (let i = 0; i < this.constants.documentTypesProviders.length; i++) {
        if (this.detail.documentType === this.constants.documentTypesProviders[i].value) {
          this.descriptionDocType = this.constants.documentTypesProviders[i].name;
        }
      }
    }
  ...
  handleRenewLists() {
    for (let i = 0; i < this.constants.documentTypesProviders.length; i++) {
      if (this.detail.documentType === this.constants.documentTypesProviders[i].value) {
        this.detail.documentType = this.constants.documentTypesProviders[i];
      } else {
      }
    }
    for (let i = 0; i < this.constants.documentExtensions.length; i++) {
  ...
  verifyAccount($event: any) {
    this.providersPaymentsService.verificationAccount($event)
