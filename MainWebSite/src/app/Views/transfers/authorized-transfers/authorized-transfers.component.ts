...
@Component({
  selector: 'app-authorized-transfers',
  templateUrl: './authorized-transfers.component.html',
  styleUrls: ['./authorized-transfers.component.css'],
  providers: [TicketsService, TransfersService, UtilsService]
})

export class AuthorizedTransfersComponent implements OnInit {
  ...
  ngOnInit() {
    this.sourceAccountDto = new AccountDto({
      operationTypeId: [OperationType.transferenciasCuentasAutorizadas],
      types: [String.fromCharCode(AccountTypes.passive)],
      accountUse: String.fromCharCode(AccountUse.debit),
      roleId: Roles.initiator
    });
    this.targetAccountDto = new AccountDto({
      accountUse: String.fromCharCode(AccountUse.credit),
      types: [String.fromCharCode(AccountTypes.passive)]
    });
    this.transferData.operationTypeId = OperationType.transferenciasCuentasAutorizadas;
  }

  validateAccounts(): boolean {
    if (this.transferData.sourceAccount === this.transferData.destinationAccount) {
      this.messageService.warning('Cuentas incorrectas', 'Las cuentas de origen y destino deben ser diferentes');
      return false;
    }
    return true;
  }
  ...
  handleValidate(currencyAndAmountValidation: boolean,
    ...
    approversAndControllersValidation: boolean, approversLimitValidation: boolean) {
    if (currencyAndAmountValidation && futureDateValidation && ticketValidation && approversAndControllersValidation &&
      this.validateAccounts() && approversLimitValidation) {
